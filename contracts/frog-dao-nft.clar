...
;; - If user holds at least 1000 FROG (from `frog-token` contract),
...
;; Helper: read FROG balance from frog-token contract.
(define-read-only (get-frog-balance (who principal))
  (unwrap-panic (contract-call? .frog-token get-balance who)))
...
(define-public (mint-pass)
  (begin
    ...
    (asserts! (>= (unwrap-panic (contract-call? .frog-token get-balance tx-sender)) min-frog-to-mint) err-insufficient-frog)
